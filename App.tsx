...
const App: React.FC = () => {
  ...
  const [lastState, setLastState] = useState<{ receipts: Receipt[]; currentIndex: number } | null>(null);
  const [canUndo, setCanUndo] = useState(false);
  const [isRejectionModalOpen, setIsRejectionModalOpen] = useState(false);
  const [rejectionReason, setRejectionReason] = useState('');

  const handleDecision = (status: ReceiptStatus, reason?: string) => {
    if (currentIndex >= receipts.length) return;
    
    // Save current state for a potential undo
    setLastState({ receipts: [...receipts], currentIndex });

    setLastAction(status === ReceiptStatus.Approved ? 'approved' : 'rejected');
    setReceipts(prevReceipts => 
      prevReceipts.map((receipt, i) => i === currentIndex ? { ...receipt, status, reason } : receipt)
    );
    setCurrentIndex(prevIndex => prevIndex + 1);
    setCanUndo(true); // Enable undo for this action
  };
  
  const handleUndo = () => {
    if (lastState) {
      setReceipts(lastState.receipts);
      setCurrentIndex(lastState.currentIndex);
      setLastAction(null);
      setLastState(null);
      setCanUndo(false); // Disable undo until the next action
    }
  };
  ...
  const handleRejectSubmit = () => {
    handleDecision(ReceiptStatus.Rejected, rejectionReason);
      ...
      complete: (results) => {
        ...
        const processedData = relevantData.map((row, index) => {
          ...
          return {
            ...
            status: status,
            reason: status === ReceiptStatus.Rejected ? 'No project submission' : undefined,
          };
        });
  ...
  const downloadResults = () => {
    const exportData = receipts.map(receipt => {
      ...
      const { status, reason, ...rest } = receipt as any;
      return {
        ...rest,
              ...
              <>
                  ...
                  <div className="mt-6 text-center text-slate-500 dark:text-slate-400">
                      ...
                      <div className="flex items-center justify-center space-x-4 h-6">
                          {lastAction && <p className="capitalize">Last Action: {lastAction}</p>}
                          {canUndo && lastAction && (
                              <button 
                                  onClick={handleUndo}
